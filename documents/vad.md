...
`ちいかわ` / `ハチワレ` / `うさぎ` は 1 パス目の ASR プロンプトに載せない。転写に `アニメ` / `あにめ` / `anime` があり、まだ正しい表記が無いときだけ 2 パス目で Preferred spellings に載せる。

雑音で `ちいかわ、ハチワレ、うさぎ` だけが出た場合（アニメを含まない）は `AsrTextFilter` が破棄する。
`はい、ありがとうございます` のような儀礼句だけの出力も、無音時の幻覚として破棄する（単独の「はい」「うん」「ありがとう」は残す）。
