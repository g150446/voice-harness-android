...
アプリには排他的な `AI対話`、`リーダー`、`Harbor` の3操作モードがある。リーダーはG2を必要とするが、
...
Harbor待機中（ライブ表示）のシングルタップは指示録音の開始/停止に使う。
...
Harbor 待機中（確認待ちでも要約・質問の表示でもない）の **シングルタップ**は、ホームの録音タップ
設定に関係なく指示録音（`capturePurpose=COMMAND`）を開始し、もう一度のシングルで停止する。
要約・質問の表示中は G2 のページ送りを優先し、シングルは録音に使わない。M5 StickC ファームには
