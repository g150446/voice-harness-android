...
アプリのチャット画面と同じ OpenClaw 会話（同じセッションキー）をG2に映す。G2は必須でなく、
有効化にはGateway tokenの設定だけが要る。ONにすると応答モデル(LLM)もOpenClawへ切り替わる
（元へは戻さない）。ボタン、音声指示「グラスモード変更 OpenClaw」、保存された前回モードから
