...
ダブルタップでモード指示録音を開始し、モード名を話して再度ダブルタップするとASRだけで
切り替える。G2未接続時のダブルタップはホームの録音タップ設定に従う。
...
端末操作は行わない。シングルタップは要約のページ送りだけに使用し、ライブ表示中は無視する。
...
**ダブルタップ**は G2 接続中の操作モード指示に使う。待機中の1回目で録音を開始し、モード名を
話した後の2回目で録音を確定する。通常の録音・文字起こし・応答・読み上げ中は中断を優先し、
直後2秒間は残留シングルタップを無視する。G2 未接続時はホームの録音タップ設定に従う。
