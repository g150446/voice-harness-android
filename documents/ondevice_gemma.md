...
Gemma / Qwen ASR プロンプトには固有名詞の希望表記リストを付与できる。

- 既定語は `AsrVocabularyCatalog.builtIn`（初期: ちいかわ / ハチワレ / うさぎ）
- これらは `triggerKeywords`（`アニメ` / `あにめ` / `anime`）付きなので、1 パス目には載せない
- 1 パス目の転写にトリガーがあり、まだ正しい表記が無いときだけ 2 パス目で載せる
- トリガー無しで常に載せたい語は `triggerKeywords` を空のまま `builtIn` に追加する
- 再ビルドなしの試験語はアプリ files 直下の `asr_vocabulary_extra.txt`  
  （1行1語。`表記` または `表記<TAB>読みヒント<TAB>説明`。`#` はコメント。extra はトリガー無し）
- 日本語 / 自動のときだけプロンプトへ載せる。英語モードには載せない
- `ちいかわ、ハチワレ、うさぎ` だけ並び、アニメが無い出力は幻覚として捨てる
- `はい、ありがとうございます` のような儀礼句だけの出力も幻覚として捨てる（単独の「はい」「うん」「ありがとう」は残す）
