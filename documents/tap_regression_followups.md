...
背景説明として残す。FW は OTA 済み、**BLE イベントの実機確認だけが未了**
（末尾の「残作業」を参照）。
...
`--sysbuild` クリーンビルドは通過済み。**`0.0.88` の OTA は 2026-08-31 に完了**し、
slot 0 で `active=true` / `confirmed=true` / version=`0.0.88` を確認した。
**以下の BLE イベントの実機確認だけが未了。**
...
OTA の記録と手順は `harness-node/docs/ota_update_notes.md` を参照。
