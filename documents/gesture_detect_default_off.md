...
上記はG2**未接続**時のみ有効。G2接続中はこのホーム設定に関係なく、録音の開始/終了は
常にダブルタップだけが担う（Harborを含む全モード共通）。シングルタップはG2接続中、何も
開始/終了しない。
M5 StickC ファームには接続時に RX `[0x08, 0x01]` を送り、単発クリックのローカル録音トグルを無効化する。
...
1. Node `0.0.95+` + 本アプリ
2. G2未接続時: シングルタップで録音 start/stop（ホーム設定でダブルタップへ変更可）。
   G2接続時: 録音 start/stop は常にダブルタップ
3. Harbor 要約表示中の single はページ送り、確認待ち中の single は実行。それ以外の
   待機中は single では何も起きない（指示録音の開始はダブルタップ）
4. ジェスチャーは発火しない
