...
- Terminal Harborで選択中のアクティブペイン末尾を、画面に収まる最大行数でライブ表示
...
3. AndroidがMacで選択中のワークスペースと画面末尾を取得し、loopback APIへ載せる。
4. プラグインは長い行を折り返し、G2領域に収まる最長の末尾部分を表示する。

Harbor表示中はスワイプやシングルタップでページ移動せず、最新末尾へ自動追従する。
初版は表示専用であり、ワークスペース切替と端末への指示入力は行わない。
