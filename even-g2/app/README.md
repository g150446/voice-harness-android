...
- AndroidのVoice Harnessで抽出したKindle本文（読書パススルー）
- Harness Nodeの**シングルタップ**でG2本文を次ページへ進める（読書モード）
...
1. パススルーをONにする（ホームのトグル、音声、または Harness Node の**ダブルタップ**）。
2. Kindleを前面にすると（起動ジェスチャーなしで）自動的に本文抽出が始まる。  
   ダブルタップ開始時は現在画面からも抽出する。ON中のダブルタップでパススルー終了。
