...
1. リーダーモードをONにする（ホームのトグル、または G2 接続中のダブルタップで「リーダーモード」と指示）。
...
2. ホームの操作モード、またはダブルタップ開始・停止の音声指示でHarborモードへ切り替える。
...
ライブ表示中はシングルタップを無視して最新末尾へ自動追従する。要約表示中に端末出力が
再開すると要約ページを破棄してライブ表示へ戻る。G2からのワークスペース切替と端末への
指示入力は行わない。
