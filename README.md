...
実行は読み上げと同時に開始する。モード切替（プラン・通常など）・workspace切替・単発キー
（Enter・Esc・Ctrl-Cなど）・Claude Code / Codex の既知のスラッシュコマンド単発（`/compact`・
`/clear`・`/model opus` など。`/exit`・`/logout` などセッション終了や認証系、未知のコマンドは除く）
のようなシンプルな指示は確認を挟まずに即実行し、実行開始と同時に「プランモードに切り替えます」
「compactコマンドを送ります」のような短いサマリーを読み上げる。自由文の指示や複数手順は従来どおり確認する。
グラスの出力モード切替は発話の先頭を
...
G2未接続でも追跡する。OpenClawが未設定・停止中の場合は同じプロンプトをGroq/OpenRouterへ送り、
それも使えない場合だけ既存のTerminal Harbor要約へフォールバックする。
